...
describe('tag', function() {
	...
	it('should merge whole tag name', function() {
		let node = dom(`<h[name|at:] class="[test]">div</hn>`);
		let copy = matchdom(node, {
			name: "div",
			test: "yes"
		});
	...
	it('should merge whole tag name when it has a parent', function() {
		let node = dom(`<div><h[name|at:] class="[test]">div</h[name|at:]></div>`);
		let copy = matchdom(node, {
			name: "main",
			test: "yes"
		});
	...
	it('should merge whole tag name when it has a next sibling', function() {
		let node = dom(`<div><h[name|at:] class="[test]">div</h[name|at:]><div class="toto"></div></div>`);
		let copy = matchdom(node, {
			name: "main",
			test: "yes"
		});
